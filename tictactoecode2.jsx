...
const Board = () => {
  ...
  function renderSquare(i) {
    ...
    return <Square takeTurn={takeTurn} id={i} doReset={doReset} ></Square>;
  }

  return (
    <div className="outerDiv">    

      <div id="info" className="score-board">       
        <h3>React Tic Tac Toe</h3>
        <br /> <br/>
        <h3 id="turn" className={player == '0' ? 'white' : 'red'}>{playerTurn}</h3>
        <br/><br/>
        <h3 id="stat" 
          className={ status == 'No Winner Yet' ? 'black' : ( status == 'Player X ' ? 'red' : 'white')}>The Winner is {status}  
        </h3>
        <br/><br/>
        <button id="bReset" className="btnReset" onClick={()=>clearState()}>Reset</button> 
        <button id="bStart" className="btnReset" onClick={()=>setDoReset(0)}>Start</button>  
      </div>
...
const Square = ({ takeTurn, id, doReset}) => {
  const mark = ['O', 'X', ''];
